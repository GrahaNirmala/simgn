...
export const GET = defineHandler(async (req) => {
  ...
  const results = await db().execute(
    ...
    FROM
        cashflow
    WHERE
        EXTRACT(MONTH FROM cashflow.created_at) = ${currentMonth}
        AND EXTRACT(YEAR FROM cashflow.created_at) = ${currentYear}
    ...
    WHERE
        ...
        AND EXTRACT(MONTH FROM payment.created_at) = ${currentMonth}
        AND EXTRACT(YEAR FROM payment.created_at) = ${currentYear}
    ORDER BY
        created_at DESC`,
  )
